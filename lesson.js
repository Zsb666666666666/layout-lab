(function () {
  const lessons = {
    ...
    flex: {
      ...
      codeDescription: { zh: "这段代码与演示的默认设置一致：从起点横向排列、不换行，以 gap 控制间距。空间不足时可以在容器内滚动。", en: "This snippet matches the demo defaults: a horizontal row starting at the beginning, without wrapping, with spacing controlled by gap. When space is tight, scroll inside the container." },
      demoDescription: { zh: "默认从起点横向排列、不换行。先调整 gap 观察间距，再切换 align-items 观察垂直对齐；窄屏可在预览区横向滚动。随后可以尝试其他分布方式和换行规则。", en: "The default is a horizontal row starting at the beginning, without wrapping. Adjust gap to see spacing change, then switch align-items to compare vertical alignment. On narrow screens, scroll inside the preview. Next, explore other distribution and wrapping options." },
      demo: "flex"
    },
    grid: {
  ...
  function renderFlexDemo(root) {
    root.innerHTML = '<div class="demo-workspace"><div class="demo-controls">' +
      ...
      controlSelect("flex-justify", "justify-content", [["flex-start", "start / 起点"], ["center", "center / 居中"], ["space-between", "space-between / 两端"], ["space-around", "space-around / 环绕"]], "flex-start") +
      ...
      '<label class="check-row"><input id="flex-wrap" type="checkbox"> wrap / 允许换行</label></div>' +
